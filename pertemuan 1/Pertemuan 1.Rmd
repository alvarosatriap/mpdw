...
library(rio) 
data1 <- import("https://raw.githubusercontent.com/alvarosatriap/mpdw/refs/heads/main/pertemuan%201/100%20row%20data.csv")
data2 <- import("https://raw.githubusercontent.com/alvarosatriap/mpdw/refs/heads/main/pertemuan%201/100%20row%20data.csv")
...
```{r}
# Set kolom Sunspot Number sebagai Yt dan Sales, serta buat kolom Periode
data1$Yt      <- data1$`Sunspot Number`
data1$Sales   <- data1$`Sunspot Number`
data1$Periode <- 1:nrow(data1)

# Samakan data2 ke data1
data2 <- data1
```
...
ggplot() + 
  geom_line(data = training_ma, aes(x = 1:nrow(training_ma), y = `Sunspot Number`, col = "Data Latih")) +
  geom_line(data = testing_ma, aes(x = (nrow(training_ma) + 1):(nrow(training_ma) + nrow(testing_ma)), y = `Sunspot Number`, col = "Data Uji")) +
...
Selanjutnya dilakukan peramalan sebanyak 24 periode sesuai dengan jumlah data uji. Pada metode SMA, seluruh hasil peramalan untuk 20 periode ke depan akan memiliki nilai yang sama dengan hasil ramalan satu periode ke depan.
...
```{r}
ts.plot(data2.ts, xlab="Time Period ", ylab="Sunspot Number", main= "SMA N=4 Data Sunspot Number")
points(data2.ts)
...
Berdasarkan hasil perhitungan akurasi pada data latih menggunakan metode Single Moving Average (SMA) dengan parameter m = 4, diperoleh nilai SSE sebesar 47167.38, MSE sebesar 620.62, dan MAPE sebesar 40.96%. Mengingat nilai MAPE berada di atas kisaran 20%–50%, kemampuan peramalan model ini pada data latih tergolong dalam kategori layak (reasonable/cukup). Selanjutnya, dilakukan perhitungan nilai akurasi pada data uji untuk mengevaluasi kinerja peramalan model SMA.
...
Perhitungan akurasi pada data uji menghasilkan nilai SSE sebesar 115471.75, MSE sebesar 5773.59, dan MAPE sebesar 153.36%. Karena nilai MAPE berada jauh di atas 50%, kemampuan peramalan model Single Moving Average (SMA) m = 4 pada data uji ini dikategorikan buruk (inaccurate). Hal ini menunjukkan bahwa metode SMA kurang mampu menangkap pola perubahan data pada periode pengujian.
...
```{r}
ts.plot(data2.ts, xlab="Time Period ", ylab="Sunspot Number", main= "DMA N=4 Data Sunspot Number")
points(data2.ts)
...
Perhitungan akurasi pada data latih menghasilkan nilai SSE sebesar 39804.62, MSE sebesar 545.27, dan MAPE sebesar 30.95%. Berdasarkan nilai MAPE yang berada di antara 20% hingga 50%, performa pemulusan Double Moving Average (DMA) pada data latih ini masuk dalam kategori cukup/layak (reasonable). Selanjutnya, perhitungan nilai akurasi dilakukan pada data uji.
...
Perhitungan akurasi pada data uji menghasilkan nilai SSE sebesar 117080.03, MSE sebesar 5854.00, dan MAPE sebesar 154.41%. Karena nilai MAPE berada jauh di atas 50%, kemampuan peramalan metode Double Moving Average (DMA) pada data uji ini dikategorikan buruk (inaccurate).

Jika dibandingkan antara kedua metode, pada data latih metode DMA menghasilkan nilai error yang lebih kecil (MAPE 30.95%) dibandingkan SMA (MAPE 40.96%). Namun pada data uji, metode SMA memiliki nilai error yang sedikit lebih rendah (MAPE 153.36%) dibandingkan DMA (MAPE 154.41%), meskipun performa kedua metode pada data uji secara umum masih kurang memuaskan.
...
#Import data
library(rio)
data3 <- import("https://raw.githubusercontent.com/alvarosatriap/mpdw/refs/heads/main/pertemuan%201/100%20row%20data.csv")
data3.ts <- ts(data3$'Sunspot Number')
```

Selanjutnya melakukan pembagian data dan mengubahnya menjadi data deret waktu.

```{r}
# Membagi data menjadi training (baris 1-80) dan testing (baris 81-100)
# Ambil langsung kolom 'Sunspot Number' (kolom ke-5)
training <- data3$`Sunspot Number`[1:80]
testing  <- data3$`Sunspot Number`[81:100]

# Ubah ke time series (misalkan frekuensi musiman = 4)
training.ts <- ts(training, frequency = 4)
testing.ts  <- ts(testing, frequency = 4, start = c(21, 1))
```
